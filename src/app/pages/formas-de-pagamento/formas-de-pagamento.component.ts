      ...
      <main class="main-content">
        <section class="page-hero">
          <div>
            ...
            <p class="hero-text">Gerencie as formas de pagamento disponíveis para associar aos pedidos.</p>
          </div>
          <div class="hero-highlights">
            <div class="hero-highlight">
              <span class="highlight-label">Total</span>
              <strong>{{ formas.length }} forma(s)</strong>
            </div>
            <div class="hero-highlight destaque">
              <span class="highlight-label">Ativas</span>
              <strong>{{ formasAtivasCount }} ativa(s)</strong>
            </div>
          </div>
        </section>
        ...
        @if (formas.length === 0 && !carregando) {
          <mat-card class="estado estado-vazio">
            <mat-card-content>
              ...
              <div>
                <strong>Nenhuma forma de pagamento cadastrada</strong>
                <p>Adicione uma forma de pagamento para que ela fique disponível na criação de pedidos.</p>
              </div>
            </mat-card-content>
          </mat-card>
                    ...
                    <td mat-cell *matCellDef="let r" class="acoes-col">
                      @if (r.ativo) {
                        <button mat-icon-button color="warn" (click)="desativar(r)"
                          matTooltip="Desativar forma de pagamento">
                          <mat-icon>block</mat-icon>
                        </button>
                      } @else {
                        <button mat-icon-button color="primary" (click)="reativar(r)"
                          matTooltip="Reativar forma de pagamento">
...
export class FormasDePagamentoComponent implements OnInit {
  ...
  carregar(): void {
    ...
    this.formasDePagamentoService.listar().subscribe({
      next: formas => {
        this.formas = formas;
      ...
      error: () => {
        this.snackBar.open('Erro ao carregar formas de pagamento.', 'OK', { duration: 4000 });
        this.carregando = false;
      }
    });
  }
  ...
  criar(): void {
    ...
    this.salvando = true;
    this.formasDePagamentoService.criar(descricao).subscribe({
      next: nova => {
        this.formas = [nova, ...this.formas];
        this.novaDescricaoControl.reset();
        this.snackBar.open('Forma de pagamento criada com sucesso.', 'OK', { duration: 3000 });
        this.salvando = false;
      },
      error: (err) => {
        this.snackBar.open(err?.message || 'Erro ao criar forma de pagamento.', 'OK', { duration: 4000 });
        this.salvando = false;
      }
    });
  }
  ...
  desativar(forma: FormaPagamento): void {
    this.formasDePagamentoService.desativar(forma.id).subscribe({
      next: atualizada => {
  ...
  reativar(forma: FormaPagamento): void {
    this.formasDePagamentoService.reativar(forma.id).subscribe({
      next: atualizada => {
        this.atualizarNaLista(atualizada);
